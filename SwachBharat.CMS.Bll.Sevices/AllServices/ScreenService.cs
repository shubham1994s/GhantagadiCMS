...
namespace SwachBharat.CMS.Bll.Services
{
    public class ScreenService : AppService, IScreenService
    {
        ...
        public SBALUserLocationMapView GetvehicalDetails(int teamId, int userId)
        {
            try
            {
                using (var db = new DevChildSwachhBharatNagpurEntities(AppID))
                {
                    ...
                    if (Details != null)
                    {
                        ...
                        loc.lat = Details.lat;
                        loc.log = Details.@long;
                        loc.VehicleList = VehicleNumberList(userId);
                        loc.userMobile = user.userMobileNumber;
                        loc.type = Convert.ToInt32(user.Type);
                        try { loc.vehcileNumber = atten.vehicleNumber; } catch { loc.vehcileNumber = ""; }
        ...
        public List<SelectListItem> VehicleNumberList(int userId)
        {
            var user = new List<SelectListItem>();
            SelectListItem itemAdd = new SelectListItem() { Text = "--Select Vehical Number--", Value = "0" };

            try
            { 
                if(userId == 0)
                { 
                user = db.Daily_Attendance.GroupBy(x => x.vehicleNumber).Select(y => y.FirstOrDefault())
                    .Select(x => new SelectListItem
                    {
                        Text = x.vehicleNumber,
                        Value = x.vehicleNumber.ToString()
                    }).OrderBy(t => t.Text).ToList();
                }
                if (userId != 0)
                {
                    user = db.Daily_Attendance.Where(x => x.userId== userId).GroupBy(x => x.vehicleNumber).Select(y => y.FirstOrDefault())
                        .Select(x => new SelectListItem
                        {
                            Text = x.vehicleNumber,
                            Value = x.vehicleNumber.ToString()
